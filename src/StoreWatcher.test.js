...
describe("get()", () => {
  test("Should able to get data from model", () => {
    ...
    model.set("a", 1);
    expect(watcher.get("a")).toBe(1);
    model.set("a", 2);
    model.set("b", 1);
    expect(watcher.get("a")).toBe(2);
    expect(watcher.get("b")).toBe(1);
  });
  ...
  test("Should get the same wrapped model", () => {
    ...
    const watcher = new StoreWatcher(model);
    const wrappedModel1 = watcher.get("a");
    const wrappedModel2 = watcher.get("a");
  ...
  test("Should get the same wrapped collection", () => {
    ...
    const watcher = new StoreWatcher(model);
    const wrappedCollection1 = watcher.get("c");
    const wrappedCollection2 = watcher.get("c");
    expect(wrappedCollection1).toBe(wrappedCollection2);
    collection.add(model);
    expect(wrappedCollection1.at(0)).toBe(watcher.store);
  });
});
...
describe(`on('change')`, () => {
  ...
  test("Should trigger change event when collection change", () => {
    ...
    model.set("c", collection);
    const watcher = new StoreWatcher(model);
    const wrappedCollection = watcher.get("c");
    const mockCallback = jest.fn();
    watcher.on("change", mockCallback);
